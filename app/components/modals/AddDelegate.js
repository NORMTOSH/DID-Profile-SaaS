...
const AddDelegate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RPC_URL;
      ...
      <div className="max-w-md w-full p-6 rounded-lg shadow-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="w-full p-2 bg-gray-800 rounded">
            <label className="block text-sm font-medium text-gray-100">Your DID</label>
            <p className="mt-1 text-gray-100 break-all">{ethrDid}</p>
          </div>
